...
EPrints::Plugin::Issues::TypeExactTitleDups

=cut

package EPrints::Plugin::Issues::TypeExactTitleDups;

use EPrints::Plugin::Issues;
...
sub new
{
	...
	$self->{name} = "Exact title and type duplicates";
...
sub process_item_in_list
{
    ...
    my $type  = $item->get_value( "type" );
    return if !defined $title || !defined $type;

    my $type_title = $type . " " . $title;
